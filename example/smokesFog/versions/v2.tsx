...
const ExplodingSphere = ({ scrollRef }: { scrollRef: React.MutableRefObject<number> }) => {
  ...
  const fragments = useMemo<FragmentData[]>(() => {
    ...
    for (let i = 0; i < innerCount; i++) {
      ...
      const scale = rScale < 0.15
        ? 0.16 + Math.random() * 0.12   // big shards
        : rScale < 0.35
    ...
    for (let i = 0; i < outerCount; i++) {
      ...
      frags.push({
        ...
        lastTarget: originalPos.clone(),
        explodeMultiplier: 0.9, // minimal drift on explosion
  ...
  useFrame((state, delta) => {
        // Rainbow glow proxy color cycle (for colored bloom)
        if (glowMatRef.current) {
          const h = (state.clock.elapsedTime * 0.08) % 1;
          glowMatRef.current.color.setHSL(h, 1.0, 0.6);
        }
    const dt = (typeof delta === 'number' ? delta : 0.016);
    const sp = THREE.MathUtils.damp(smoothProgressRef.current, scrollRef.current, 3.5, dt);
    smoothProgressRef.current = sp;
    if (groupRef.current) {
      groupRef.current.rotation.x = state.clock.elapsedTime * 0.08 * (1 - sp);
      groupRef.current.rotation.y = state.clock.elapsedTime * 0.12 * (1 - sp);
      const breathe = 1 + Math.sin(state.clock.elapsedTime * 0.8) * (0.03 + sp * 0.02);
    ...
    fragmentsRef.current.forEach((mesh, i) => {
      ...
      const explodeFactor = elasticOut(sp) * (frag.explodeMultiplier ?? 2);
      ...
      const floatOffset = normal.multiplyScalar(
        Math.sin(t * frag.floatFrequency + frag.phase) * frag.floatAmplitude * (0.5 + sp)
      );
      ...
      mesh.rotation.x = frag.rotation.x + sp * frag.rotationSpeed.x * 0.85;
      mesh.rotation.y = frag.rotation.y + sp * frag.rotationSpeed.y * 0.85;
      mesh.rotation.z = frag.rotation.z + sp * frag.rotationSpeed.z * 0.85;
      const matAny = mesh.material as any;
      if (matAny) {
        const opacityVal = (1 - sp * 0.7) * (frag.opacityFactor ?? 1);
        if (matAny.uniforms) {
  ...
  return (
    ...
    >
      <group ref={groupRef}>
        {fragments.map((frag, i) => (
          ...
          >
            <meshStandardMaterial
              ...
              metalness={frag.isOuter ? 0.6 : 0.9}
              roughness={frag.isOuter ? 0.25 : 0.1}
              transparent
              opacity={frag.isOuter ? 0.3 : 1}
              envMapIntensity={frag.isOuter ? 0.2 : 0.8}
            />
          </mesh>
        ))}
        {/* Core sphere hidden; keep light only */}
        <mesh ref={coreRef} visible={false} position={[0, 0, 0]}>
        ...
        {/* Center rainbow glow proxy removed per request */}
        {/* Central sphere light restored at 40% */}
        <pointLight position={[0, 0, 0]} intensity={0.4} distance={8} decay={2} color="#ffffff" />
        {/* Atmosphere shells */}
        <mesh scale={1.7 * (1 - smoothProgressRef.current * 0.5)}>
          <sphereGeometry args={[1, 32, 32]} />
          <meshBasicMaterial
            color="#4a90d9"
            transparent
            opacity={0.08 * (1 - smoothProgressRef.current)}
            side={THREE.BackSide}
          />
        </mesh>
        <mesh rotation={[Math.PI / 2, 0, 0]} scale={1 + smoothProgressRef.current * 0.5}>
          <torusGeometry args={[2.2, 0.01, 16, 100]} />
          <meshBasicMaterial color="#4a90d9" transparent opacity={0.3 * (1 - smoothProgressRef.current)} />
        </mesh>
      </group>
    </Float>
  );
...
const Scene = ({ scrollRef }: { scrollRef: React.MutableRefObject<number> }) => {
  ...
  useFrame((state) => {
    const t = state.clock.elapsedTime;
    const h = (t * 0.08) % 1; // slow hue cycle
    if (rainbowLightRef.current) {
      rainbowLightRef.current.color.setHSL(h, 0.7, 0.5);
    }
  });
  return (
    <>
      ...
      <directionalLight position={[-5, -5, -5]} intensity={0.3} color="#4a90d9" />
      {/* Scene point light re-enabled at ~10% brightness */}
      <pointLight ref={rainbowLightRef} position={[0, 0, 3]} intensity={0.06} color="#4a90d9" />
      {/* Central point light moved inside ExplodingSphere for dynamic control */}
      ...
      {/* Subtle Bloom to test glow without causing flashes */}
      <EffectComposer>
        <Bloom
